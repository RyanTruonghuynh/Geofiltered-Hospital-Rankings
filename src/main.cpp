...
//Option 1 needs a find function that returns a boolean if search was successful
//Not sure if it works  without any bugs until the find or search for city function is impemented

//Option 2 will create and store a vector of weights (5 weights in the vector) as a global variable. Should be used for
//calculating average weight or however we plan to find the score

//Option 3 - Need to know how top 5 hospitals will be stored so I can iterate and display them. Also, either include the
//information (average score, city, state initial, name of hospital) for each hospital in the same data structure
//or they will be called through some search or find function idk

//Option 4 - Placeholders in brackets need to be replaced with actual values
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::KeyPressed) {
                if (event.key.code >= sf::Keyboard::Num1 && event.key.code <= sf::Keyboard::Num7) {
                    ...
                    //CHOICE 1 - Set Location - Needs a function to find the input city should return a boolean, the city name
                    //and state, and coordinates.
                    if (choice == 1) {
                        sf::Event clearEvent;
                        while (window.pollEvent(clearEvent)) {}
                        //inputName - user input of city name
                        //inputDistance - user input of distance
                        //cityName - City name that is stored once user input is valdiated
                        //nameFound - City name has been found - used for find function
                        string inputName = "";
                        ...
                        while (Option1 && window.isOpen()) {
                            ...
                            while (window.pollEvent(Opt1Event)) {
                                ...
                                if (Opt1Event.type == sf::Event::KeyPressed && Opt1Event.key.code == sf::Keyboard::Enter) {
                                    ...
                                    if (!inputName.empty() && !nameFound) {
                                        //City name will be searched through a find function from the data structure chosen
                                        //ex) geohashTable.findCity(inputName, nameFound);
                                        //inputName is user input and nameFound is boolean to see if input is valid
                                        //Or maybe make it so the findCity returns a boolean:
                                        //nameFound = findCity(inputName);
                                ...
                                if (!nameFound && Opt1Event.type == sf::Event::TextEntered) {
                                    ...
                                    //checks a reasonable length for a city name
                                    else if (inputName.length() < 25 && isalpha(Opt1Event.text.unicode)) {
                                    ...
                                    else if (Opt1Event.text.unicode == ' ' && !inputName.empty() &&
                                             inputName.back() != ' ') {
                                        inputName += ' ';
                                    }
                                }

                                // Handle distance input - only allow digits 0-9 (no decimal points)
                            ...
                            if (!nameFound  && !nameEnter) {
                                ...
                                if (inputName.empty()) {
                                    inputDisplay.setString("Type Here");
                                } else {
                            ...
                            } else if (!inputName.empty() && !nameFound && nameEnter) {
                                // City was not found
                                sf::Text error(  inputName + " not found in database", font, 30);
                                ...
                                sf::Text errorInstruction("Please type a different city name", font, 20);
                                ...
                                sf::Text prompt2("Enter maximum distance (miles) from " + cityName + ":", font, 30);
                                ...
                                // Distance input text
                                sf::Text distanceDisplay;
                                if (inputDistance.empty()) {
                                    distanceDisplay.setString("Type Here");
                                } else {
                                    distanceDisplay.setString(inputDistance);
                                }
                                distanceDisplay.setFont(font);
                                distanceDisplay.setCharacterSize(28);
                            ...
                            //Everything was successful
                                ...
                                // **start geohash filtering here based on name of city and distanec **
                            }
                    ...
                    if (choice == 2) {
                        ...
                        while (Option2 && window.isOpen()) {
                            ...
                            while (window.pollEvent(weightEvent)) {
                                ...
                                // only allow user input to be a number between 0 and 5. If backspace, remove last digit
                                // of inputstring
                                if (weightEvent.type == sf::Event::TextEntered) {
                                    if (weightEvent.text.unicode >= '0' && weightEvent.text.unicode <= '5'){
                    ...
                    //For option 3 I was thinking of maybe calling vector<struct> hospitals
                    // the struct will contain the name of the hospital, city, State initials, and score
                    if (choice==3) {
                        sf::Event clearEvent;
                        while (window.pollEvent(clearEvent)) {}
                        bool Option3 = true;
                        while (Option3 && window.isOpen()){
                            ...
                            while (window.pollEvent(Opt3Event)) {
                                ...
                                sf::Text subtitle("Top 5 hospitals based on your input preference", font, 24);
                                subtitle.setFillColor(sf::Color::Blue);
                                sf::FloatRect subtitleBounds = subtitle.getLocalBounds();
                                subtitle.setPosition((1200 - subtitleBounds.width) / 2, 100);
                                ...
                                rankCol.setPosition(100, 160);
                                window.draw(rankCol);

                                sf::Text hospitalCol("Hospital Name", font, 28);
                                hospitalCol.setFillColor(sf::Color::Black);
                                hospitalCol.setStyle(sf::Text::Bold);
                                hospitalCol.setPosition(200, 160);
                                window.draw(hospitalCol);

                                sf::Text cityCol("| City", font, 28);
                                cityCol.setFillColor(sf::Color::Black);
                                cityCol.setStyle(sf::Text::Bold);
                                cityCol.setPosition(700, 160);
                                window.draw(cityCol);

                                sf::Text scoreCol("| Score", font, 28);
                                scoreCol.setFillColor(sf::Color::Black);
                                scoreCol.setStyle(sf::Text::Bold);
                                scoreCol.setPosition(900, 160);
                                window.draw(scoreCol);

                                // !! Create interface for the hospital rankings !!
                                //Need to know how top 5 hospitals will be stored so i can iterate and display



                                sf::RectangleShape topBorder(sf::Vector2f(1000, 2));
                                topBorder.setFillColor(sf::Color::Black);
                                topBorder.setPosition(100, 200);
                                window.draw(topBorder);

                                sf::RectangleShape bottomBorder(sf::Vector2f(1000, 2));
                                bottomBorder.setFillColor(sf::Color::Black);
                                bottomBorder.setPosition(100, 530);
                                ...
                                if (selectedHospitalInd >= 0 && selectedHospitalInd <= 4 && citySelected) {
                    ...
                    //Option 5 creates a file that has the top 5 ranked hospitals based on inputs, the weights the user
                    //inputted, and the top recommended hospital
                    if (choice == 5) {
                        sf::Event clearEvent;
                        while (window.pollEvent(clearEvent)) {}
                        bool Option5 = true;
